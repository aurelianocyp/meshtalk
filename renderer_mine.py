...
class Renderer:
    ...
    def render(self, verts: th.Tensor):
        ...
        if not os.path.exists("results"):
            os.makedirs("results")
        #save_obj(os.path.join("results", 'obj_{}.obj'.format(len(os.listdir("results")))), verts, self.faces.expand(verts.shape[0], -1, -1))
        IO().save_mesh(mesh, os.path.join("results", 'obj_{}.obj'.format(len(os.listdir("results")))))
        with th.no_grad():
    ...
    def to_video(self, verts: th.Tensor, audio_file: str, video_output: str, fps: int = 30, batch_size: int = 30):
        ...
        images = th.cat([self.render(v).cpu() for v in th.split(verts, 1)], dim=0)
       # for v in th.split(verts, batch_size):
        #    if not os.path.exists("results"):
         #       os.makedirs("results")
          #  obj_filename=os.path.join("results", 'obj_{}.obj'.format(len(os.listdir("results"))))
           # save_obj(obj_filename, faces=v)
